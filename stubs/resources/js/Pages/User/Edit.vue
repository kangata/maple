...
const create = async () => {
  ...
  try {
    const { data } = await axios.put(route('json.users.update', props.user.id), input.value)
    ...
    <Content>
      <div class="bg-white rounded-lg shadow p-6">
        ...
        <MapleFormItem label="Password" class="mb-4" :error="errors?.password">
          <ElInput v-model="input.password" size="large" type="password" placeholder="Leave blank if you don't want to change it" show-password />
        </MapleFormItem>
        ...
        <div class="mt-8 flex justify-end">
          <ElButton type="primary" :loading="loading" @click="create">UPDATE USER</ElButton>
        </div>
      </div>
    </Content>
  </AppLayout>
